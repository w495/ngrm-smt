...
randomMatrix(N, UpperBound,Seed) ->
    ...
    F = fun(Key,M) ->
        R = round(random:uniform() * UpperBound),
...
parallelMain() ->
    ...
    % Pid3 =
    spawn(ant,parallelWork,[Seed,3,Boost,Iter,NumCities,self()]),
    % Pid4 =
    spawn(ant,parallelWork,[Seed,4,Boost,Iter,NumCities,self()]),
    ...
    receive
    {BestPath3,BestPathLength3} ->
        io:write(BestPath3),
        io:fwrite(" : ",[]),
        io:write(BestPathLength3),
        io:fwrite("\n",[])
    end,
    receive
    {BestPath4,BestPathLength4} ->
        io:write(BestPath4),
        io:fwrite(" : ",[]),
        io:write(BestPathLength4),
        io:fwrite("\n",[])
     end.
